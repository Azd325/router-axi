...
`router-axi` is a local, agent-ergonomic router CLI. The supported surface is read-only TR-064 inspection. Do not add mutations, browser scraping, telemetry, credential persistence, or model-name-based capability guesses without an approved design change.
...
- Keep live-router tests opt-in. They require `ROUTER_AXI_LIVE_TEST=1`, explicit credentials and host, and must refuse CI.
